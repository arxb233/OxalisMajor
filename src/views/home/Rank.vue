...
import { GetLogoUrl, GetRank } from '@/api/Major'
...
const getList = async () => {
    loading.value = true
    try {
        const res = await GetRank(yearValue.value, subjectValue.value)
   ...
   const numMatch = val.match(/^\d+$/)
    if (numMatch) {
        ...
        visibleList.value = fullData.value.filter(item => {
            const score = parseInt(item.分数)
            return !isNaN(score) && score >= center - 20 && score <= center + 20
        })
    } 
    finished.value = true
})
...
<template>
    <div class="search d-flex flex-column justify-content-start">
        <van-sticky>
            <CustomHeader title="分段" leftIcon="arrow-left" />
            ...
            <form action="javascript:return true">
                <van-search v-model="value" placeholder="请输入分数" shape="round" :autofocus="false">
                    <template #action>
        ...
        <van-list v-model:loading="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
            <div v-for="item in visibleList" :key="item.id" style="padding-bottom: 12px;">
                <van-card :thumb="logoList.find(i => i.name === item.学校)?.url || logo" thumb-link="#">
                    ...
                    <template #title>
                        <div style="display: flex; flex-direction: column; margin-bottom: 6px;">
                            ...
                            <div
                                ...
                                批次：<van-tag type="primary" plain>{{ item.批次 }}</van-tag>
                            </div>
                        </div>
                    </template>
                    ...
                    <template #bottom>
                        <div
                            style="display: flex; align-items: center; flex-wrap: wrap; font-size: 14px; color: #2196F3; margin-top: 3px ; gap: 6px;">
                            <span>历史同位次考生得分：{{ item.历史 }}</span>
                        </div>
                    </template>
                </van-card>
            </div>
