...
import { GetLogoUrl, GetPlan } from '@/api/Major'
...
const getList = async () => {
    loading.value = true
    try {
        const res = await GetPlan(yearValue.value, subjectValue.value)
...
watch(value, async (val) => {
   ...
   visibleList.value = fullData.value.filter(item =>
            ...
            item.录取批次.includes(val) ||
            item.招生专业.includes(val)
        )
    await GetLogo()
    finished.value = true
})
...
<template>
    <div class="search d-flex flex-column justify-content-start">
        <van-sticky>
            <CustomHeader title="招生" leftIcon="arrow-left" />
                    ...
                    <template #title>
                        <div style="display: flex; flex-direction: column; margin-bottom: 6px;">
                            ...
                            <div
                                ...
                                录取批次：<van-tag type="primary" plain>{{ item.录取批次 }}</van-tag>
                            </div>
                        </div>
                    </template>
